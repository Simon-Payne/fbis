...
```
A `standalone` profile enables the server to be run standalone, managing its own SSL connection security.
A `heroku` profile is provided for running in a cloud hosted context where its security is provided by the host. 
